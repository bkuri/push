...
Push should support several named agents inside one assistant. A named agent is
a stable Push configuration that selects an agent runtime and an optional
...
Push dispatches the target runtime, records the relationship, and returns the
result to the caller. Push does not decide how to solve the task and does not
...
This changes Push from selecting one backend into coordinating a small,
...
`reviewer`. An inbound Telegram message is routed to `neo`. Push starts Neo's
...
When the reviewer finishes, Push stores a bounded result, marks the child
...
Push may select an optional runtime-native profile but does not interpret its
...
or tool schema in Push was rejected because it would couple the gateway to
fast-changing runtime APIs.

### Delegation uses a CLI control surface

The first version uses `push agent run`, called through the runtime's existing
shell tool. This works across coding agents without making Push an MCP host or
requiring a new agent protocol. A Push-owned MCP delegation server and ACP
client are possible later surfaces over the same internal dispatcher, but are
not required for the first version.

### The caller chooses; Push only dispatches

Push supplies the allowed agent names and descriptions. The caller decides
whether to delegate and writes the child request. Push never classifies the
...
also has a maximum depth. Push enforces these rules for correctly propagated
...
Push does not retry a delegated runtime after failure or timeout because the
...
8. Push never configures or proxies a runtime's domain tools, credentials, or
   external services.
...
- Each agent has a non-empty description suitable for showing to an allowed
  delegator.
- Push appends delegation instructions only when the active agent has allowed
  targets.
...
`runtime_profile` is optional and opaque to Push. An adapter must either map it
...
`push agent run` reads one non-empty prompt from standard input. Prompts larger
...
64 KiB with an explicit truncation marker. Push accepts at most 1 MiB of live
final reply data from a delegated runtime. A larger reply fails with
`output_too_large`; Push does not return a partial answer to the parent.
...
name, config path, config hash, and depth through reserved `PUSH_` environment
...
Invocation ids are Push-generated UUIDs. The first invocation in a chat turn,
...
gateway starts. `push doctor` reports all catalog errors in one pass.
...
If a parent exits while children are active, Push marks those children
`cancelled`, terminates their backend processes, and retains any already
completed child rows. If Push crashes, the next command or service start marks
...
- No implementation code introduces Gmail, external-service OAuth, a generic
  tool registry, an MCP host, or an agent reasoning loop inside Push.
...
- An agent may delegate a poor or dangerous prompt. The target runtime's own
  permissions remain the enforcement boundary, and Push records the full
...
- Gmail, calendar, browser, or other external-service integrations in Push.
- Model, tool, skill, MCP, credential, or permission configuration shared by
  Push.
- Push choosing which agent should handle a subtask.
